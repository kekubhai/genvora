 ...
 */
export function initTracer(serviceName, options) {
    const endpoint = options?.endpoint ?? process.env["OTEL_EXPORTER_OTLP_ENDPOINT"] ?? "http://localhost:4317";
    const logLevel = process.env["OTEL_LOG_LEVEL"] === "debug"
        ? DiagLogLevel.DEBUG
